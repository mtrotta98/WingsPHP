...
<html>
    ...
    <body class="hold-transition sidebar-mini layout-fixed">
                            ...
                            <div class="col-md-12">
                                <div class="box-body">
                                    <table id="example1" class="table table-bordered table-striped" style="font-size: 12px">
                                        ...
                                        <tbody>
                                            <?php
                                            $sql = "SELECT * FROM curso_docente_alumnos WHERE id_docente=$id";
                                            $res_existe = $mysqli->query($sql);
                                            while($row = $res_existe->fetch_array(MYSQLI_ASSOC)){
                                                
                                                $id_curso = $row['id_curso'];

                                                $sql2 = "SELECT * FROM cursos WHERE id_curso=$id_curso";
                                                ...
                                                while ($row2 = $existe->fetch_array(MYSQLI_ASSOC)) {
                                                    $i++;
                                                ?>
                                                <tr>
                                                    <td style="text-align: center;"> <?php echo $i ?></td>
                                                    <td style="text-align: left;"> <?php echo $row2['nombre_curso'] ?></td>
                                                    <td style="text-align: center;"> <?php echo $row2["diaD"] . " A " . $row2["diaH"] ?></td>
                                                    <td style="text-align: center;"> <?php echo $row2["horaIn"] . " A " . $row2["horaFin"] ?></td>
                                                    <td><a onclick="alumnos(<?php echo $id_curso;?>)"><i class="fa fa-fw fa-edit" style="font-size: 25px;;cursor:pointer;"></i></a></td>
                                                    <td><a onclick="libros(<?php echo $id_curso; ?>)"><i class="fa fa-fw fa-trash" style="font-size: 25px;;cursor:pointer;"></i></a></td>
        ...
        <script>
            function alumnos(id) {
                var id = id;
                window.location = "listado_alumnos.php?id=" + id
            }

            function libros(id){
                var id = id;
                window.location = "listado_libros.php?id=" + id
            }
