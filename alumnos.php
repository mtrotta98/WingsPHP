...
<html>
    ...
    <body class="hold-transition sidebar-mini layout-fixed">
            ...
            <div class="content-wrapper">
                <div class="content-header">
                    <div class="container-fluid">
                        <div class="row mb-2">
                            <div class="col-sm-6">
                                <h1 class="m-0 text-dark">Listado de Alumnos</h1>
                            </div>  
                        </div>
                    </div>
                </div>
                                    ...
                                    <table id="example1" class="table table-bordered table-striped" style="font-size: 12px">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width: 15px;text-align:center">ASIGNAR CURSO</th>
                                                <th style="width: 5px;">EDITAR</th>
                                                <th style="width: 5px;">BORRAR</th>
                                                <th style="width: 15px; text-align:center">AGREGAR PAGO</th>
                                                <th style="width: 5px; text-align:center">AGREGAR FICHA</th>
                                                <?php //} ?>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php
                                            $sql = "SELECT apellido,nombre,id_personas,fecha_nacimiento FROM alumnos WHERE activo=1 order by apellido,nombre";
                                            ...
                                            while ($row = $res_existe->fetch_array(MYSQLI_ASSOC)) {
                                                $i++;
                                                $date_nacimiento = $row['fecha_nacimiento'];
                                                $date = new DateTime($date_nacimiento);
                                                $now = new DateTime();
                                                $edad = $now->diff($date)->format('%Y');
                                                $id_p = $row['id_personas'];
                                                $sql2 = "SELECT MAX(p.fecha) AS ultimoPago FROM pagos p INNER JOIN cuenta_corriente cc ON (p.id_cc = cc.id_cc) WHERE cc.id_persona=$id_p";
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="text-align: center;"> <?php echo $row2['ultimoPago'] ?></td>
                                                    <td><a onclick="curso(<?php echo $row['id_personas']; ?>)"><i class="fa fa-book" style="font-size: 25px;;cursor:pointer;"></i></a></td>
                                                    ...
                                                    <td><a onclick="pago(<?php echo $row['id_personas']; ?>)"><i class="fa fa-plus" style="font-size: 25px;cursor:pointer; text-align:center;"></i></a></td>
                                                    <td><a onclick="agrega_ficha(<?php echo $row['id_personas']; ?>)"><i class="fa fa-file" style="font-size: 25px;cursor:pointer; text-align:center"></i></a></td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
        ...
        <script>
            ...
            function pago(id){
                var id = id;
                window.location = "pagos.php?id=" + id
            }

            function agrega_ficha(id){
                var id = id;
                window.location = "carga_ficha_medica.php?id=" + id
            }
