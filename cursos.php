...
<html>
    ...
    <body class="hold-transition sidebar-mini layout-fixed">
                            ...
                            <div class="col-md-12">
                                <div class="box-body">
                                    <table id="example1" class="table table-bordered table-striped" style="font-size: 12px">
                                        ...
                                        <tbody>
                                            <?php
                                            $sql = "SELECT * FROM cursos WHERE activo=1 order by nombre_curso";
                                            ...
                                            while ($row = $res_existe->fetch_array(MYSQLI_ASSOC)) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="text-align: left;"> <?php echo $row["nombre_curso"] ?></td>
                                                    <?php
                                                        if ($row['diaD'] == $row['diaH']){
                                                            ?>
                                                            <td style="text-align: center;"> <?php echo $row["diaD"]?></td>
                                                            <?php
                                                        }else{
                                                            ?>
                                                            <td style="text-align: center;"> <?php echo $row["diaD"] . " Y " . $row["diaH"] ?></td>
                                                            <?php
                                                        }
                                                    ?>
                                                    <td style="text-align: center;"> <?php echo $row["horaIn"] . " Y " . $row["horaFin"] ?></td>
                                                    <td><a onclick="edita(<?php echo $row['id_curso']; ?>)"><i class="fa fa-fw fa-edit" style="font-size: 25px;;cursor:pointer;"></i></a></td>
                                                    <td><a onclick="borra(<?php echo $row['id_curso']; ?>)"><i class="fa fa-fw fa-trash" style="font-size: 25px;;cursor:pointer;"></i></a></td>
        ...
        <script>
            function edita(id) {
                var id = id;
                window.location = "edita_cursos.php?id=" + id
            }
            ...
            function borra(id) {
                ...
                $.ajax({
                    type: "POST",
                    url: "acciones.php?valor=7&accion=consulta",
                    data: {
                        id: id
                    },
                    success: function (data) {
                        ...
                        if (mensaje) {
                            $.ajax({
                                type: "POST",
                                url: "acciones.php?valor=7&accion=eliminar",
                                data: {
                                    id: id
                                },
                                success: function (data) {
